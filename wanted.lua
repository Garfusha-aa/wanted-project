...
-- ?? ПРОВЕРКА LOADER
local loaderPath = getWorkingDirectory().."\\dt_loader.lua"
do
    local f = io.open(loaderPath, "r")
    if not f then
        sampAddChatMessage("{00FFFF}District Tracker | {FF0000}Запуск скрипта невозможен. Code: 210!", -1)
        thisScript():unload()
        return
    end
    f:close()
...
local renderFont = renderCreateFont("Arial", 12, 5) -- увеличил размер
...
local function checkAccess(nickname)
    ...
    if code ~= 200 then
        sampAddChatMessage("{00FFFF}District Tracker | {FF0000}Ошибка проверки доступа!", -1)
        return false
    end
...
function main()
    ...
    if not checkAccess(nickname) then
        sampAddChatMessage("{00FFFF}District Tracker | {FF0000}У тебя нет доступа!", -1)
        sampAddChatMessage("{00FFFF}District Tracker | {98FB98}Обратись к разработчику", -1)
        thisScript():unload()
        return
    end

    sampAddChatMessage("{00FFFF}District Tracker | {98FB98}Доступ разрешён: "..nickname, -1)
    ...
    for line in text:gmatch("[^\n]+") do
        ...
        if line:lower():find("wanted") then
            ...
            if nickname and playerId then
                ...
                sampAddChatMessage(
                    "{00FFFF}District Tracker | {98FB98}Цель: " .. targetName .. " [" .. targetId .. "]",
                    -1
                )
            end
        end
        ...
        if allowedTextDraws[id] then
            for _, data in ipairs(zones) do
                ...
                if line:lower():find(name:lower(), 1, true) then
                    ...
                    sampAddChatMessage("{00FFFF}District Tracker | {98FB98}Район: " .. name, -1)
                    return
                end
